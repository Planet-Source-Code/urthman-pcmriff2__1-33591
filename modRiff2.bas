...
' AmplitudeMod: This routine applies an attenuation value against
'   a given wave based on corresponding wave data from another
'   sine wave array
'
Sub AmplitudeMod(Which%, AM_ID%)

'   Which = which sine wave array
'   AM_ID = The sine wave to provide amplitude information

Dim AdjustBy As Double
Dim aIndx&

    For aIndx = 0 To UBound(Wave(Which).Sample)
        AdjustBy = Wave(AM_ID).Sample(aIndx) / ((2 ^ bWidth) - 1)
...
End Function
' MakeSineFM calculates modulated sine wave values against the sample
'   values of another sine wave array
'
Function MakeSineFM(Which%, ByVal Freq As Double, ByVal MS As Long, ByVal FM_ID%) As Boolean

'   Which = identifies waveform array (use sequentially starting with ZERO)
'   Freq = Frequency; cycles per second
'   MS = Milliseconds in duration
'   FM_ID = The wave sample array that provides the modulation values
...
' CALCULATE THE PhaseShift BASED ON THE corresponding Wave(FM_ID).Sample values
        
        FreqShift = Wave(FM_ID).Sample(Wave(Which).Index)
